## Gitのミニレポート
### Q3-1. Git同様のバージョン管理システムの1つにSubversionがある。Subversionの特徴および問題点を説明せよ。
* 特徴
* ・集中型バージョン管理システム
* ・サーバーにインストールしたリモートリポジトリが
* 　ソースコードのバージョンを管理している
* 問題点
* ・Gitに比べて様々な処理の速度が遅い
* ・正しくマージされない
* ・Gitに比べて複雑
### Q3-2. Subversionの問題点をGitではどのように解決しているか説明せよ。
* Subversionの問題点
* ・完全には機能しない、途中段階のコードは一切コミットできない
* Gitにおける解決方法
* ・ローカルコミットを用いることで、途中段階でもコミットが可能になった
### Q3-3. Gitの機能の中で、複数人でのシステム開発において、特に重要となる機能を1つ選び説明せよ。
* 複数人でのシステム開発で重要となるGitの機能
* ・Gitリポジトリ
* その理由（具体的な状況とその機能を使った役割分担など）
* ・ファイルやディレクトリを保存して置けるものであり、リモートリポジトリと
* 　ローカルリポジトリを使うことで、複数人で進捗を共有しつつ各自で
* 　開発を進めることが可能になる
